...
export default function MatafuegoEntregaWizard({
  ...
  return (
    <div className={`mf-entrega-wizard-root${step > 1 ? ' mf-entrega-wizard-active' : ''}${step === 2 ? ' mf-entrega-wizard-pick' : ''}`}>
      ...
      {step === 1 && (
        <div className="mf-entrega-step mf-entrega-step--deps">
          <div className="mf-entrega-card mf-entrega-card--deps">
            <h3 className="mf-entrega-step-title">Paso 1 · Seleccionar destino</h3>
            <p className="mf-entrega-step-desc">
              Buscá por nombre, código o número. Se listan <strong>todas</strong> las coincidencias; hacé clic en la fila o en <strong>Seleccionar</strong>.
            </p>
            ...
            {depRows.length > 0 && (
              <p className="mf-entrega-search-hint" role="status">
                {depRows.length} coincidencia{depRows.length === 1 ? '' : 's'}
                {depSearchActive ? ' — ordenadas de más a menos relevante' : ''}
                {depSearchActive && !/\d/.test(busquedaDep) && ' · Probá «comisaria 1», «com 4» o «4ta»'}
                {depRows.length > 6 && ' · Usá la barra de desplazamiento de la tabla ↓'}
              </p>
            )}
            <div
              className="mf-entrega-deps-scroll-host"
              aria-label="Lista de dependencias encontradas"
            >
            <div className="mf-entrega-table-wrap mf-entrega-table-wrap--scroll">
